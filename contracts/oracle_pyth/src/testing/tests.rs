...
use crate::testing::mock_fn::{CREATOR, mock_instantiate, mock_instantiate_msg, PYTH_CONTRACT};
...
fn test_instantiate() {
    ...
    let res = change_pyth_contract(deps.as_mut(), new_info.clone(), "new_pyth_contract".to_string());
    assert!(res.is_ok());
    let config = query_config(deps.as_ref()).unwrap();
...
fn test_update_config_feed_info() {
    ...
    // config feed info
    let price_feed_id = "ff61491a931112ddf1bd8147cd1b641375f79f5825126d665480874634fd0ace".to_string();
    ...
    let config_feed_info_res = config_feed_info(deps.as_mut()
                                            , info.clone(), asset.clone(),
    ...
    let feeder_config =
        query_pyth_feeder_config(deps.as_ref(), asset.clone()).unwrap();
